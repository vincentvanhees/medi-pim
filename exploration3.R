...
data = data_light_glasses |> LightLogR::data2reference(data_light_wrist, PIM, Reference.column = PIM_wrist)
data <- data |>
  dplyr::select(Id, Datetime, MEDI, PIM)
...
output2 = aggregate(output[, c("time", "MEDI", "PIM", "changepoint")], by = list(output$time, output$Id), FUN = mean)
output2 = output2[, -which(names(output2) %in% c("Group.1"))]
names(output2)[1] = "Id"

x11()
plot_changepoint_ts(output2, "low PIM => high PIM")
